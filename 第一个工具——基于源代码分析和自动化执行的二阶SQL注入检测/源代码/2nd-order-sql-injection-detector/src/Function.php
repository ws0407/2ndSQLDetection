...
function analyseFunction($func_name){
    return 0;
}
...
function fetchVariableList($file_name, $var_list_tmp, &$var_list){
    foreach ($var_list_tmp as $var){
        ...
        if($var == "_POST" || $var == "_GET" || $var == "_FILE")
            $var_info[2] = "web";
        elseif ($var == "_SESSION")
            $var_info[2] = "db";
        $flag = false;
...
function analyseGlobalVariable($name, $dim, $dir, $var_list){
    $left_index = findIndex($name.$dim, $dir, $var_list);
    $list = searchGlobalVariableLocation($dim);
    $var_right_name = $list[0];
    $var_right_dir = $list[1];
    $var_right_index = findIndex($var_right_name, $var_right_dir, $var_list);
    if($var_list[$var_right_index][2] != "" && ($var_list[$var_right_index][2][1] == 'w' ||
            $var_list[$var_right_index][2][1] == 'd')){
        $var_list[$left_index][2] = $var_list[$var_right_index][2];
    }elseif($var_list[$var_right_index][2] != "" && $var_list[$var_right_index][2][1] == 'p'){
        $var_list[$left_index][2] = "php";
    }
}

function searchGlobalVariableLocation($name){
    return [$name, 1];
}
